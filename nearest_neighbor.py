...
def find_3_smallest(a):
    indexed = []
    for i, elem in enumerate(a):
        pair = (i, elem)
        indexed.append(pair)
    #print(f"Sorted: {indexed}")
    indexed.sort(key=lambda t: t[1])
    return list(map(lambda t: t[0], indexed[:3]))
...
def main(X_train, X_test, y_train, y_test):
    ...
    for i, test_item in enumerate(X_test):
        ...
        k3_nearest = find_3_smallest(distances)

        #print(f"K3 Nearest: {k3_nearest}")
        ...
        predictions_nearest = []
        for j in k3_nearest:
            predictions_nearest.append(y_train[j])

        y_predict[i] = 0 if sum(predictions_nearest) <=1 else 1         # this just classifies everything as 0
